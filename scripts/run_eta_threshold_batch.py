...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--outdir",
        type=Path,
        default=Path("out/paper_artifacts/eta_threshold"),
        help="Study output dir.",
    )
    parser.add_argument(
        "--artifacts-dir",
        type=Path,
...
def aggregate_results(
    ...
    if artifacts_dir.resolve() != outdir.resolve():
        agg.to_csv(artifacts_path, index=False, float_format="%.6f")
...
def write_macros(
    ...
    eta_sig: float | None,
    bk_at_equal: float | None,
    eqt_at_equal: float | None,
    delta_equal: float | None,
) -> None:
    if eta_equal is None:
        sentence = "本実験範囲では，EQTがBKと同等になるηは確認できなかった．"
    else:
        sentence = (
            "EQTがBKと同等になる最小効率は\\EtaEqualであり，"
            "そのときBK=\\BKAvailAtEtaEqual，EQT=\\EQTAvailAtEtaEqual"
            "（差\\DeltaAvailAtEtaEqual）である．"
        )
    ...
    lines = [
        ...
        f"\\newcommand{{\\BKAvailAtEtaEqual}}{{{format_metric_value(bk_at_equal)}}}",
        f"\\newcommand{{\\EQTAvailAtEtaEqual}}{{{format_metric_value(eqt_at_equal)}}}",
        f"\\newcommand{{\\DeltaAvailAtEtaEqual}}{{{format_metric_value(delta_equal)}}}",
        f"\\newcommand{{\\EtaThresholdSentence}}{{{sentence}}}",
    ]
    path.write_text("\n".join(lines) + "\n", encoding="utf-8")
...
def write_readme(
    ...
    if insensitivity_note:
        lines.extend(["", "Eta sensitivity note", f"- {insensitivity_note}"])
    lines.extend(
        [
            "",
            "Run command (example)",
...
def write_eta_usage_note(path: Path) -> None:
    src_path = ROOT / "sequence" / "qn" / "strategy_params.py"
    lines = src_path.read_text(encoding="utf-8").splitlines()

    def find_line(needle: str) -> int | None:
        for idx, line in enumerate(lines, start=1):
            if needle in line:
                return idx
        return None

    eqt_line = find_line('strategy == "EQT"')
    eqt_calc = find_line("base_prob * eta * eta")
    dqt_line = find_line('strategy == "DQT"')
    dqt_calc = find_line("base_prob * eta")
    bk_line = find_line("else:  # BK / optical baseline")
    bk_calc = find_line("base_prob)")

    note_lines = [
        "eta usage note",
        f"- file: {src_path}",
        "- function: edge_params_for_strategy",
        "- EQT uses eta^2 scaling (base_prob * eta * eta).",
        "- DQT uses eta scaling (base_prob * eta).",
        "- BK does not include eta (base_prob only).",
        "",
        "Line references (1-based)",
        f"- EQT branch: line {eqt_line}, calc line {eqt_calc}",
        f"- DQT branch: line {dqt_line}, calc line {dqt_calc}",
        f"- BK branch: line {bk_line}, calc line {bk_calc}",
    ]
    path.write_text("\n".join(note_lines) + "\n", encoding="utf-8")
...
def main() -> int:
    ...
    topology = baseline.get("topology", "nsfnet")
    derived_root = args.outdir / "derived" / topology
    raw_root = args.outdir / "raw" / topology
    ...
    if not args.aggregate_only:
        ...
        example_bk = build_sim_command(
            ...
            / topology
            / "BK"
            / f"eta_{format_eta(bk_eta)}"
            / f"seed_{args.seed_start}",
            baseline=baseline,
        )
        ...
        for eta in etas:
            for scenario in scenarios:
                ...
                for seed in iter_seeds(args.seed_start, args.seed_end):
                    ...
                    raw_dir = raw_root / scenario / f"eta_{eta_label}" / f"seed_{seed}"
                    raw_dir.mkdir(parents=True, exist_ok=True)
                    raw_csv = raw_dir / "entanglement_service_raw.csv"
                    raw_json = raw_dir / "summary.json"
    ...
    if results_path:
        ...
        bk_at_equal = lookup_metric(agg, "BK", eta_equal, "mean_success_rate") if eta_equal else None
        eqt_at_equal = lookup_metric(agg, "EQT", eta_equal, "mean_success_rate") if eta_equal else None
        delta_equal = None
        if bk_at_equal is not None and eqt_at_equal is not None:
            delta_equal = eqt_at_equal - bk_at_equal

        eqt_rows = agg[agg["scenario"] == "EQT"]
        insensitivity_note = None
        if not eqt_rows.empty:
            eqt_min = float(eqt_rows["mean_success_rate"].min())
            eqt_max = float(eqt_rows["mean_success_rate"].max())
            span = eqt_max - eqt_min
            insensitivity_note = (
                f"EQT mean range across eta: {eqt_min:.4f}..{eqt_max:.4f} (Δ={span:.4f})."
            )
            if span < 1e-4:
                insensitivity_note += " Variation is tiny; likely bottleneck or p_eg floor."
        else:
            insensitivity_note = "EQT rows missing in aggregated results."
        ...
        readme_path = args.artifacts_dir / "README.md"
        write_readme(
            readme_path,
            etas,
            baseline,
        ...
        write_eta_usage_note(args.artifacts_dir / "eta_usage_note.txt")
