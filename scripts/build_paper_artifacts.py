...
def collect_topology_sweep(derived_root: Path, rows: list[dict[str, object]]) -> None:
    for summary_path in derived_root.rglob("summary.csv"):
        rel = summary_path.relative_to(derived_root)
        if len(rel.parts) < 5:
            continue
        topology = rel.parts[0]
        scenario = rel.parts[1]
        eta = float(parse_tag(rel.parts[2], "eta_"))
        k = int(parse_tag(rel.parts[3], "k_"))
        seed = int(parse_tag(rel.parts[4], "seed_"))
        success_rate, n_requests = read_summary_csv(summary_path)
        rows.append(
            {
                "study": "study_topology_sweep",
                "topology": topology,
                "scenario": scenario,
                "eta": eta,
                "k": k,
...
def write_readme(path: Path, agg: pd.DataFrame, out_root: Path) -> None:
    ...
    topology_raw = out_root / "study_topology_sweep" / "raw"
    if topology_raw.exists():
        raw_notes.append(f"- topology sweep raw CSVs: {topology_raw}")
    ...
    topo_subset = agg[agg["study"] == "study_topology_sweep"]
    if not topo_subset.empty:
        topo_list = sorted(set(str(t) for t in topo_subset["topology"]))
        lines.extend(["", "Topology sweep", f"- topologies: {', '.join(topo_list)}."])
    lines.append("")
...
def main() -> int:
    ...
    topology_sweep_dir = out_root / "study_topology_sweep" / "derived"
    ...
    if topology_sweep_dir.exists():
        collect_topology_sweep(topology_sweep_dir, rows)
    ...
    df_master = df[
        ["study", "topology", "scenario", "eta", "k", "seed", "success_rate"]
    ].sort_values(
        ["study", "topology", "scenario", "eta", "k", "seed"]
    )
    df_master.to_csv(artifacts_dir / "results_master.csv", index=False)

    agg = (
        df.groupby(["study", "topology", "scenario", "eta", "k"], as_index=False)
        .agg(
            n_seeds=("seed", "count"),
        ...
        .sort_values(["study", "topology", "scenario", "eta", "k"])
    )
